...
import NovaLanguageSelector from './components/nova/NovaLanguageSelector';
...
export default function App(){
 ...
 return <div className="app-shell">
  <header className="topbar"><a className="brand" href="/" aria-label="SynexAgent 홈"><span className="brand-icon"><Activity size={24}/></span><span>Synex<span className="brand-light">Agent</span></span><span className="version">CLINICAL WORKSPACE</span></a><div className="top-meta"><NovaLanguageSelector/><span className="demo-label">{patient?(patient.is_synthetic?t('env.syntheticDemo'):t('env.fhirPatient',{source:patient.data_source})):t('env.none')}</span><span className="top-sep"/><span className="user-avatar">DR</span><span>{t('topbar.reviewMode')}</span></div></header>
  <aside className="patient-sidebar"><div className="workspace-label">{t('patient.workspaceLabel')} <span>01</span></div><div className="sidebar-title"><h2>{t('patient.listTitle')}</h2><span>{(patients.length+customPatients.length).toString().padStart(2,'0')}</span></div><label className="searchbox"><Search size={17}/><input aria-label={t('patient.search')} placeholder={t('patient.search')} value={query} onChange={e=>setQuery(e.target.value)}/></label><button className="new-patient-button" onClick={()=>setCustomOpen(true)}><UserPlus size={16}/> {t('patient.newPatient')}</button>{patientsUnavailable&&<div className="fhir-lookup"><p className="muted small">{t('patient.fhirRosterUnavailable')}</p><label className="searchbox"><Search size={17}/><input aria-label={t('patient.fhirLookupPlaceholder')} placeholder={t('patient.fhirLookupPlaceholder')} value={fhirLookup} onChange={e=>setFhirLookup(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&fhirLookup.trim()){setSelected(fhirLookup.trim());setFhirLookup('')}}}/></label></div>}<div className="list-caption">{t('patient.todaysCases')} <Users size={15}/></div><nav aria-label="환자 선택" className="patient-list">{visible.map(p=><div key={p.id} className={'patient-row '+(selected===p.id?'selected':'')}><button className="patient-row-btn" onClick={()=>setSelected(p.id)} aria-current={selected===p.id?'true':undefined}><div className="patient-row-top"><span className="patient-avatar">{p.name.slice(0,1)}</span><div><b>{p.name}</b><small>{p.age}{t('patient.years')} · {p.sex==='female'?t('patient.female'):t('patient.male')} · {p.id}</small>{p.custom&&<span className="custom-tag">{t('patient.customTag')}</span>}</div><ChevronRight size={17}/></div><div className="patient-row-bottom"><span>{p.scenario.split(' · ')[0]}</span><span className={'risk-dot '+(p.risk.risk_level==='high'?'red':'teal')}/></div></button>{p.custom&&<button className="patient-row-delete" aria-label={t('patient.deleteAria',{name:p.name})} onClick={()=>removeCustomPatient(p.id)}><Trash2 size={14}/></button>}</div>)}</nav>{!visible.length&&<p className="muted empty">{t('patient.noResults')}</p>}<div className="sidebar-foot"><ShieldCheck size={20}/><b>{t('patient.sidebarFootTitle')}</b><p>{t('patient.sidebarFootBody')}</p><span>SynexAgent / MVP 1.0</span></div></aside>
  <main className="main-workspace">
   <div className="breadcrumb">{t('workspace.breadcrumb')} <ChevronRight size={13}/> {t('workspace.patientAnalysis')} <span className="mono">{selected}</span></div>
   {error&&<div className="error-banner" role="alert"><TriangleAlert size={18}/><span>{error}</span><button onClick={()=>{setBoot(v=>v+1);setRevision(v=>v+1)}}>{t('common.retry')}</button></div>}
   <div className="page-heading"><div><div className="eyebrow">{t('workspace.patientOverview')}</div><h1>{patient?.name||t('workspace.loadingPatient')}<span>{patient?`${patient.age}${t('patient.years')} / ${patient.sex==='female'?t('patient.female'):t('patient.male')}`:''}</span></h1><p>{patient?.diagnosis||t('workspace.loadingRecords')}</p></div><button className="outline-button" disabled={busy} onClick={()=>setRevision(v=>v+1)}><RotateCw size={15} className={busy?'spin':''}/> {t('workspace.reanalyze')}</button></div>
   <div className="patient-facts"><div><Pill size={17}/><span>{t('workspace.factsMeds')}</span><b>{active.length}<small>{t('workspace.unit.kinds')}</small></b></div><div><HeartPulse size={17}/><span>{t('workspace.factsConditions')}</span><b>{patient?.conditions.length??'—'}<small>{t('workspace.unit.count')}</small></b></div><div><ShieldCheck size={17}/><span>{t('workspace.factsAllergies')}</span><b className={patient?.allergies.length?'text-danger':''}>{patient?.allergies.length??'—'}<small>{t('workspace.unit.count')}</small></b></div><div><FlaskConical size={17}/><span>{t('workspace.factsRecentLabs')}</span><b className="date-value">{patient?.labs.at(-1)?.date.slice(5)||t('workspace.noInfo')}</b></div></div>
   {!isCustom&&<PatientClinicalHeader patient={patient} encounters={encounters}/>}
   <div className="tabbar" role="tablist" aria-label="환자 보기">{[['overview',t('tabs.overview'),BrainCircuit],['timeline',t('tabs.timeline'),Clock],['clinical-note',t('tabs.clinicalNote'),ClipboardCheck],['orders',t('tabs.orders'),Pill],['results',t('tabs.results'),FlaskConical],['medication',t('tabs.medication'),Database],['anatomy',t('tabs.anatomy'),Layers],['records',t('tabs.records'),FileText],['nova',t('tabs.nova'),BrainCircuit],['audit',t('tabs.audit'),History]].map(([id,label,Icon])=><button role="tab" id={'tab-'+id} aria-selected={tab===id} aria-controls={'panel-'+id} key={id} className={tab===id?'active':''} onClick={()=>{setTab(id);if(id==='audit'&&!customRef.current.some(c=>c.id===selected))refreshAudit()}}><Icon size={16}/>{label}{id==='audit'&&<span>{audit.length}</span>}</button>)}</div>
   ...
   {tab==='nova'&&<NovaWorkspace patientId={isCustom?null:selected} encounterId={undefined}/>}
